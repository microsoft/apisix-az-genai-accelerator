...
local function resolve_msi_endpoint(conf)
    return conf.msi_endpoint
        or os.getenv("MSI_ENDPOINT")
        or os.getenv("IDENTITY_ENDPOINT")
        or default_msi_endpoint
end

local function build_msi_headers(conf)
    local secret = conf.msi_secret
        or os.getenv("MSI_SECRET")
...
local function fetch_token(conf)
    ...
    if not res then
        core.log.error(plugin_name .. ": MSI HTTP error", " endpoint=", msi_endpoint, " has_secret=", has_secret, " err=", err)
        return nil, "msi_http_error:" .. (err or "unknown")
    end
    if res.status < 200 or res.status > 299 then
        core.log.error(plugin_name .. ": MSI status ", res.status, " endpoint=", msi_endpoint, " has_secret=", has_secret)
